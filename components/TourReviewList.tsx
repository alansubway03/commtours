...
import type { AgencyReviewSummary } from "@/lib/data/reviews";
...
export function TourReviewList({
  ...
  tourId: string;
  agency: string;
  summary: AgencyReviewSummary;
}) {
  return (
    <Card>
      <CardHeader>
        <h3 className="text-lg font-semibold">{agency} 會員評分總覽</h3>
      </CardHeader>
        ...
        {summary.total === 0 ? (
          ...
          <>
            <div className="grid gap-2 text-sm sm:grid-cols-2">
              ...
              <p>工作人員：{renderScore(summary.guideAvg)} / 5</p>
              <p>性價比：{renderScore(summary.valueAvg)} / 5</p>
            </div>
            <Button variant="secondary" asChild className="w-full sm:w-auto">
              <Link href={`/tours/${tourId}/reviews`}>查看此旅行社全部會員評價（附圖文詳情）</Link>
            </Button>
          </>
        )}
      </CardContent>
